# TODO: refactor into renderer service object?
module MarkdownHelper
  def render_lesson_markdown(text)
    ...
    html = transform_html_repl_blocks(html)
  ...
  def transform_html_repl_blocks(html)
    doc = Nokogiri::HTML::DocumentFragment.parse(html)
    ...
    doc.css('pre.repl > code').each do |code_node|
      ...
      # Output iframe
      output_iframe = Nokogiri::XML::Node.new('iframe', doc)
      output_iframe['data-repl-target'] = 'output'
      output_iframe['class'] = 'repl-output'
      container.add_child(output_iframe)
      ...
      run_button = Nokogiri::XML::Node.new('button', doc)
      run_button['type'] = 'button'
      run_button['data-action'] = 'repl#run'
      run_button['class'] = 'repl-run btn btn-primary btn-sm'
      run_button.content = 'Run'
      action_row.add_child(run_button)

      reset_button = Nokogiri::XML::Node.new('button', doc)
      reset_button['type'] = 'button'
      reset_button['data-action'] = 'repl#reset'
      reset_button['class'] = 'repl-reset btn btn-outline-secondary btn-sm ms-2'
