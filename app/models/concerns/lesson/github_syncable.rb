...
module Lesson::GithubSyncable
  ...
  def sync_content_from_github
    ...
    rewrite_markdown_image_asset_paths
    rewrite_html_img_asset_paths
    rewrite_html_video_asset_paths
  ...
  def rewrite_markdown_image_asset_paths
  ...
  def rewrite_html_img_asset_paths
    raw_base = "https://raw.githubusercontent.com/#{github_repository_path}/#{github_repository_branch}/"

    # Replace <img src="assets/..."> with full GitHub raw URL
    self.content.gsub!(/<img([^>]*?)src=["'](assets\/[^"']+)["']([^>]*)>/i) do
      pre_attrs = Regexp.last_match(1)
      asset_path = Regexp.last_match(2)
      post_attrs = Regexp.last_match(3)
      %Q(<img#{pre_attrs}src="#{raw_base}#{asset_path}"#{post_attrs}>)
    end
  end

  def rewrite_html_video_asset_paths
